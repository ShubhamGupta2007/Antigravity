...
export default async function Home() {
  ...
  const isPlanner = dbUser?.role === 'planner'
  ...
  const features = [
    ...((isAdmin || isPlanner) && !isEffectivelyGuest ? [
  ...
  return (
    <main className="min-h-screen flex flex-col p-6 max-w-5xl mx-auto bg-ivory pb-20">
      ...
      <header className="relative w-full border-4 border-double border-marigold/40 rounded-3xl p-6 md:p-10 bg-white/60 backdrop-blur-md shadow-md flex flex-col items-center justify-center text-center mt-2 mb-10 overflow-hidden">
        ...
        <span className="text-xs uppercase tracking-widest text-maroon/60 font-data mb-1">Shubh Vivah</span>
        <h1 className="text-3xl md:text-5xl font-display font-medium text-maroon tracking-normal leading-tight mb-1">
          Swati & Satyam
        </h1>
        <p className="text-base md:text-lg font-display italic text-maroon/80 tracking-wide mb-4">
          Are getting married!
        </p>
      ...
      {dbUser?.role === 'pending' ? (
        <section className="flex flex-col items-center justify-center space-y-6 mt-12 bg-white/80 p-8 rounded-3xl border border-marigold/30 shadow-sm max-w-2xl mx-auto text-center">
          <div className="p-4 bg-marigold/20 rounded-full">
            <Users className="w-10 h-10 text-marigold" />
          </div>
          <h2 className="text-2xl font-display font-medium text-maroon">
            Your account is pending admin approval.
          </h2>
          <p className="font-data text-maroon/70 max-w-md">
            Please wait while the admins review your request. You will be able to access the dashboard once approved.
          </p>
        </section>
      ) : dbUser?.role === 'denied' ? (
        <section className="flex flex-col items-center justify-center space-y-6 mt-12 bg-white/80 p-8 rounded-3xl border border-red-200 shadow-sm max-w-2xl mx-auto text-center">
          <div className="p-4 bg-red-50 rounded-full">
            <Bell className="w-10 h-10 text-red-400" />
          </div>
          <h2 className="text-2xl font-display font-medium text-red-700">
            Access Request Declined
          </h2>
          <p className="font-data text-maroon/70 max-w-md">
            The admins have declined your access request. Please try again after sometime, or contact the family for more information.
          </p>
        </section>
      ) : (
        <>
