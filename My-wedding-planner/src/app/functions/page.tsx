...
export default async function FunctionsPage() {
  ...
  // Fetch functions
  const { data: functions } = await supabase.from('functions').select('*').order('event_date')

  // Filter based on role and side (Temporary until RLS is fully locked down)
  const visibleFunctions = (functions || []).filter(f => {
    if (role === 'admin') return true
    return f.hosting_side === side || f.hosting_side === 'joint'
  }) as FunctionItem[]

  const canAdd = role === 'admin' || side
